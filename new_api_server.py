...
# Constants
MODEL_PATH = "output/models_20250321_222124/lstm_cnn_roberta_full.pt"
MODEL_VOCAB_PATH = "output/models_20250321_222124/lstm_cnn_roberta_full_vocab.json"
...
class ModelManager:
    ...
    def load_model(self):
        ...
        try:
            ...
            from src.models.lstm_cnn_classifier import LSTM_CNN_Classifier
            ...
            self.model = LSTM_CNN_Classifier(
                vocab_size=metadata['vocab_size'],
                embedding_dim=metadata['embedding_dim'],
                lstm_hidden_size=metadata.get('hidden_dim', 256),
                cnn_out_channels=metadata.get('cnn_out_channels', 128),
                num_classes=2,
            ...
            # Check which tokenizer to use
            if 'tokenizer_name' in vocab_data:
                tokenizer_name = vocab_data['tokenizer_name']
                if tokenizer_name == 'albert-base-v2':
                    self.tokenizer = AutoTokenizer.from_pretrained('albert-base-v2')
                    logger.info("Using Albert tokenizer")
                else:
                    # Default to Albert tokenizer if specified but not recognized
                    self.tokenizer = AutoTokenizer.from_pretrained('albert-base-v2')
                    logger.info(f"Tokenizer '{tokenizer_name}' not recognized, using Albert tokenizer")
            else:
                # Fallback to Albert tokenizer if not specified
                self.tokenizer = AutoTokenizer.from_pretrained('albert-base-v2')
                logger.info("No tokenizer specified, using Albert tokenizer")
    ...
    def predict(self, text: str) -> Tuple[bool, float]:
        ...
        try:
            ...
            # Get prediction - use forward method that takes both inputs
            with torch.no_grad():
...
async def root():
    ...
    return {
        "status": "online",
        "model": "LSTM-CNN Classifier with Albert tokenizer",
...
if __name__ == "__main__":
    print(f"Starting Advanced Complaint Detection API server with LSTM-CNN Classifier model...")
